  ...
  private static SimpleChannelInboundHandler<FullHttpRequest> getStatefunInboundHandler() {
    ...
    return new SimpleChannelInboundHandler<FullHttpRequest>() {
      ...
      protected void channelRead0(
        ...
        res.whenComplete(
            (completeSlice, throwable) -> {
              if (throwable != null) {
                throw new IllegalStateException(
                    "Exception occurred handling a test request", throwable);
              }
              FullHttpResponse response =
                  new DefaultFullHttpResponse(
                      HttpVersion.HTTP_1_1,
                      HttpResponseStatus.OK,
                      Unpooled.copiedBuffer(completeSlice.toByteArray()));
              response.headers().set(HttpHeaderNames.CONTENT_TYPE, "application/octet-stream");
              response.headers().set(HttpHeaderNames.CONTENT_LENGTH, completeSlice.readableBytes());
