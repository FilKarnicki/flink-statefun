...
public class NettyClientTest extends TransportClientTest {
    ...
    NettyClient nettyClient = NettyClient.from(
        ...
        () -> new ChannelDuplexHandler() {
          ...
          public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
            final FullHttpResponse response = (msg instanceof FullHttpResponse) ? (FullHttpResponse) msg : null;
            if (response != null) {
              statusCodeFuture.complete(response.status().code());
            } else {
              statusCodeFuture.completeExceptionally(new IllegalStateException("the object received by the test is not a FullHttpResponse"));
            }
          ...
          public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) {
            ...
            final ByteBuf bodyBuf = serializeProtobuf(ctx.channel().alloc()::buffer, request.toFunction());
            DefaultFullHttpRequest http = new DefaultFullHttpRequest(HttpVersion.HTTP_1_1, HttpMethod.POST, request.uri(), bodyBuf, new DefaultHttpHeaders(), NettyHeaders.EMPTY);
  ...
  private NettyRequestReplySpec createSpec(String trustedCaCerts, String clientCerts, String clientKey, String clientKeyPassword) {
  ...
  private Boolean callUsingStubsAndCheckSuccess(NettyClientWithResultStatusCodeFuture nettyClientAndStatusCodeFuture) throws Throwable {
    NettyRequest nettyRequest = new NettyRequest(nettyClientAndStatusCodeFuture.nettyClient, getFakeMetrics(), getStubRequestSummary(), getEmptyToFunction());
    nettyRequest.start();
  ...
  private static class NettyClientWithResultStatusCodeFuture {
    ...
    public NettyClientWithResultStatusCodeFuture(NettyClient nettyClient, CompletableFuture<Integer> resultStatusCodeFuture) {
